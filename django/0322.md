...
- `login()` 
  1. 현재 세션에 연결하려는 인증된 사용자가 있는 경우 login() 함수로 `request객체`와 `User 객체`를 통해 로그인 진행. 
...
- AuthenticationsForm을 확인해보면 `forms.form`을 상속받고 있다. 얘는 인증해서 session만 만들면 되는거니까 modlform일 필요가없다. 또 modelform들은 항상 data가 더 먼저 온다. 반면 authenticationsform 생성자를 보면 첫번째 인자로 request가 들어간다. 뒤는 data가 들어간다.
...
- 현재 로그인한 유저가 누구인지 볼 수 있다. username이 출력된다. 현재 admin, 세션을 지우고 보면 AnonymousUser
...
- 위에서 auth 덕분에 그냥 user라고 해도 보이지만 명시적으로 보기 위해 request.user라고 사용했다.
...
- 사실 이렇게 작성하면 어색한 부분이있다. 로그인하지 않았는데 로그아웃 버튼이 바로 보이게 된다. 따라서 수정이 필요하다.
...
- `is_authenticated` attribute (메서드 아님!)
  1. User class의 속성(attributes)
  2. 사용자가 인증되었는지 확인하는 방법
  3. `User에 항상 True`며, `AnonymousUser에 대해 항상 False`
  4. 권한과 관련 없고 사용자가 활성화 상태이거나 유효한 세션을 가지고 있는지도 확인하지 않음. 오직 로그인여부만 판단한다.
...
- 그런데 이 경우에 버튼은 안보이지만 url로 접근할 수 있다.
...
- 원래 action에 무조건 accounts:login으로 가도록 되어있었다. 따라서 비워서 현재 url로 다시 요청을 보내게 만든다.
...
- 여기서 request가 이미 포함되어있기 때문에 우리가 request.user를 사용할 수 있다. (이외에도 모든 템플릿에서 사용하는 request들도)
...
- UserCreationForm은 modelForm. 비밀번호를 입력하면 자동으로 set_password()를 통해 비밀번호 자동생성
...
- 탈퇴해도 세션이 지워지진 않는다. 어차피 그 사람이 탈퇴된거니까 큰 상관은 없다.
