...
```bash
python manage.py startapp articles(폴더 이름)
```
...
- Variable routing : 주소 자체를 변수처럼 사용해서 동적으로 주소를 만드는 것. 매개변수 태그를 넣을 수 있다. 기본값은 str이기 때문에 str은 생략해서 쓸 수도 있다. ex) 'articles/<int:year >/<ing:month >/' 

  - slug : string인데 아스키문자, 숫자, -, _  로 이루어져있다.

  

views.py (variable routing 사용했을 때)
...
- 이 태그를 만날 때 마다 인자를 생성한다.  첫 번째 인자는 첫 만남에서 만들어지고 두 번째 인자는 두 번째 만남에서 만들어진다. 모든 인자가 만들어졌다면 다시 첫 인자가 만들어진다. 반복문에서 효과적으로 쓰인다.
- 예시 : 반복문동안 table row가 cycle을 통해 두가지 class로 만들어진다.
- 또한 row1, 2대신 변수로 받아온 값을 class명으로 사용하는 것도 가능 (변수 아닌 것과 섞어서도 가능.)
