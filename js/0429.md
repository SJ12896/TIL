...
- 참조Reference 타입 : 객체 타입의 자료형들. 변수에 해당 객체의 참조 값이 담김. 다른 변수에 복사할 때 참조 값이 복사됨. / 함수(Function), 배열(Arrays), 객체(Objects)
...
- `for each` : 내용물 하나씩 꺼내서 출력 / break, continue 사용 불가능
...
- Arrow Function : function 키워드 생략 가능. 매개변수가 단 하나라면 ( )도 생략 가능. 함수 바디가 표현식 하나라면 { }, return도 생략 가능
